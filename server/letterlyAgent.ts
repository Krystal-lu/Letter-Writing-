...
import { GenerateRequestPayload, GenerateResponsePayload, RewriteRequestPayload, RewriteResponsePayload } from "../src/types.js";
  ...
  for (const model of modelsToTry) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        // If high demand / 503 / rate limit, wait slightly and retry or switch model
...
`.trim();
  ...
  // Fallback safety if model chose clarification when forceDraft was set
